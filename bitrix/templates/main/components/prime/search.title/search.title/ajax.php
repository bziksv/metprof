		...
		<div class="polimer-search-dropdown__products">
			...
			<ul class="polimer-search-dropdown__list polimer-search-dropdown__list--products">
				<?foreach($products as $arItem):
					$buyProductId = (int)($arItem['OFFER_ID'] ?? $arItem['ELEMENT_ID'] ?? 0);
				...
				<li class="polimer-search-item polimer-search-item--product polimer-search-item--<?=htmlspecialcharsbx($stockStatus)?>"
					...
					<div class="polimer-search-dropdown__product-row">
						...
						<div class="polimer-search-dropdown__actions">
							...
							<button type="button"
								class="polimer-search-dropdown__action polimer-search-dropdown__action--cart<?if(!empty($arItem['IN_BASKET'])):?> is-in-cart<?endif?>"
								title="<?=!empty($arItem['IN_BASKET']) ? 'Перейти в корзину' : 'В корзину'?>"
								<?if(!empty($arItem['IN_BASKET'])):?>
								onclick="window.location.href='<?= SITE_DIR ?>personal/cart/';">
								<?else:?>
								onclick="addToBasket2(<?=$buyProductId?>, 1, this, 5);">
								<?endif?>
