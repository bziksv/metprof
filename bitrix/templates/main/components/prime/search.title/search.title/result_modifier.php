...
if (CModule::IncludeModule('sale')) {
	...
	if ($fuserId) {
		...
		while ($basketRow = $basketRes->Fetch()) {
			$basketOfferIds[(int)$basketRow['PRODUCT_ID']] = true;
		}
	}
}
...
foreach ($arResult["CATEGORIES"] as $category_id => &$arCategory)
{
	...
	foreach ($arCategory["ITEMS"] as $i => $arItem)
	{
		...
		$offerId = (int)($data['OFFER_ID'] ?? $productId);
		$arResult["CATEGORIES"][$category_id]["ITEMS"][$i]['OFFER_ID'] = $offerId;
		$arResult["CATEGORIES"][$category_id]["ITEMS"][$i]['IN_BASKET'] = !empty($basketOfferIds[$offerId]);
