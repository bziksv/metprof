			...
			<? foreach($arResult['USER_FIELD'] as $field):?>
				...
				<?if($field['PROPERTY_TYPE'] == "S"):?>
					...
					<span class="line cl">
     					...
     					<span class="value"><input type="text" <?if($field["IS_REQUIRED"]=='Y'):?>required="required"<?endif;?> placeholder="<?if($field['CODE'] == "PHONE"){print "+7 (473) 234-03-01";}else{print $field['NAME'];}?>" class="<?if($field['CODE'] == "PHONE"){print "phone";}elseif($field['CODE'] == "FIO"){print "name";}?>" name="<?=$field['CODE']?>" value="<?=$arResult[$field['CODE']]?>"/></span>
     				</span>
...
<div class="g-recaptcha" data-sitekey="6LeUbdsqAAAAAEPIqSZ3jxKfu3PI4OYMfeTvjC-K"></div>
			...
			<span class="line submit">
                ...
                <input type="submit" name="submit" class="roi_buy_one_click_cart" value="<?=GetMessage("MFT_SUBMIT")?>">
			</span>
