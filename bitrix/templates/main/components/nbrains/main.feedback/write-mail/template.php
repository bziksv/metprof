	...
	<? foreach($arResult['USER_FIELD'] as $field):?>
		...
		<?if($field['PROPERTY_TYPE'] == "S" AND $field['CODE'] != "DESC"):?>
			...
			<span class="line cl">
     			...
     			<span class="value"><input <?if($field["IS_REQUIRED"]=='Y'):?>required="required"<?endif;?> type="text" placeholder="<?if($field['CODE'] == "PHONE"){print "+7 (473) 234-03-01";}else{print $field['NAME'];}?>" class="<?if($field['CODE'] == "PHONE"){print "phone";}elseif($field['CODE'] == "FIO"){print "name";}?>" name="<?=$field['CODE']?>" value="<?=$arResult[$field['CODE']]?>" /></span>
     		</span>
