...
<?elseif(count($arResult["SEARCH"])>0):?>
  ...
  <?
        ...
        <?
            $arItem["TITLE_FORMATED"] = htmlspecialcharsBack($arItem["TITLE_FORMATED"]);
            if( $arItem['PICTURE'] ){?>
    ...
    <div class="pro-title">
        ...
        <P class="search-body"><?=$arItem['BODY_FORMATED']?></P>

        <div class="clear"></div>
            <?
            if (count($arItem['CHAIN'])>0)
            {
                $arItem['CHAIN'] = array_reverse($arItem['CHAIN']);
                $arChain = array(
                    "<a href='/catalog/'>Каталог</a>"
                );
                $arChain = array_merge($arChain, $arItem['CHAIN']);
                echo '<div class="search_chain">'.implode(" / ",$arChain).'</div>';
            }
            ?>
    <hr />
  <?endforeach;?>
