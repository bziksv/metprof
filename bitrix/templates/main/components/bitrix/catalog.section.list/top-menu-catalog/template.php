					...
					<? foreach($three_sect[$i] as $sec_2): ?>
						...
						<li >
							...
							<? foreach($sec_2['SECTION_2'] as $sec_3): 
								if (! $arResult['PROPERTY']['UF_DELETE_INDEX']) {
									$arResult['PROPERTY']['UF_DELETE_INDEX'] = [];
								}
								?>
                                <? if(in_array($sec_3['ID'], $arResult['PROPERTY']['UF_DELETE_INDEX'])):?>
                                    <li><span class="delete-for-search-index" data-id="<?=$sec_3['ID']?>"></span></li>
                                <? else: ?>
