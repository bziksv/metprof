$(document).ready(function() {
   ...
   */
    ...
    */
    ...
    if($('.js-text').length){
        $('.js-text').each(function (i, v) {
            var el = $(v);
            el.html(el.data('text'));
        });
    }

    $('.tag_menu-item').popover({
        trigger : 'hover',
        animation : false,
        content : function () {
            if($(this).find('.tag_menu-prompt').length)
                return $(this).find('.tag_menu-prompt').text();
            else
                return false;
        }
    })

    if($('.delete-for-search-index').length){
        $('.delete-for-search-index').each(function (i, v) {
            $.get( "/ajax/get_section.php", { id: $(v).data('id') }, function( data ) {
                if(data.status){
                    $('<a/>', {
                        "class": 'list-item u-js',
                        "href": data.SECTION_PAGE_URL,
                    }).text(data.NAME).insertAfter($(v));
                    $(v).remove();
                }
            }, "json");
        });
    ...
    var $headerBottom = $("header .header__bottom");
    if ($headerBottom.length) {
        var headerBottomFixedAt = $headerBottom.offset().top + 60;
        $(document).scroll(function () {
            if ($(this).scrollTop() >= headerBottomFixedAt) {
