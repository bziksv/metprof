...
(function () {
	...
	function switchToLogin() {
		...
		// Оформление заказа: форма входа слева
		var orderLogin = document.querySelector('.form_login input[name="USER_LOGIN"]')
			|| document.querySelector('form[name="ORDER_AUTH_FORM"] input[name="USER_LOGIN"]')
			|| document.querySelector('.or__content input[name="USER_LOGIN"]');
		if (orderLogin) {
			orderLogin.focus();
			if (orderLogin.scrollIntoView) {
				orderLogin.scrollIntoView({ block: 'center', behavior: 'smooth' });
			}
		}
	}
	...
	function showDuplicate(message, accounts, title, opts) {
		...
		wrap.innerHTML = '<div class="prime-phoneauth-modal__overlay" data-close="' + overlayClose + '"></div>'
			...
			+ '<button type="button" class="prime-phoneauth-modal__close" data-close="1" aria-label="Закрыть">&times;</button>'
	...
	function findRegisterPhoneContext() {
		...
		// Регистрация на оформлении заказа
		var orderForm = document.querySelector('form[name="order_reg_form"]');
		if (orderForm) {
			var orderPhone = orderForm.querySelector('input[name="USER_PERSONAL_PHONE"]');
			if (orderPhone) {
				var orderLine = orderPhone.closest('.line');
				if (orderLine && !orderForm.querySelector('.prime-phoneauth-reg')) {
					return { form: orderForm, phoneInput: orderPhone, anchor: orderLine, mode: 'order' };
				}
			}
		}
	...
	function initRegister() {
		...
		box.className = 'prime-phoneauth-reg'
			+ (ctx.mode === 'bx' ? ' prime-phoneauth-reg--bx' : '')
			+ (ctx.mode === 'order' ? ' prime-phoneauth-reg--order' : '');
		...
		function applyLookup(data, phone, forcePopup) {
			if (!data || !data.ok) {
				clearRegDuplicateNotice();
				statusEl.className = 'prime-phoneauth-reg__status';
				statusEl.textContent = '';
			...
			lastLookupData = data;
			if ((ctx.mode === 'bx' || ctx.mode === 'order') && phoneDuplicateActive(data)) {
				phoneInput.classList.add('is-invalid');
				openRegDuplicatePopup(data, phone, !!forcePopup);
				return;
			}
			clearRegDuplicateNotice();
			if (lookupOnly) {
		...
		form.addEventListener('submit', function (e) {
			...
			if (lastLookupData && phoneDuplicateActive(lastLookupData)) {
				...
				e.stopPropagation();
				phoneInput.classList.add('is-invalid');
				applyLookup(lastLookupData, phoneInput.value, true);
			...
			if (!lastLookupData || lastLookupPhone !== phoneInput.value) {
				...
				postForm(cfg.lookupUrl, { phone: phoneInput.value }).then(function (data) {
					lastLookupPhone = phoneInput.value;
					applyLookup(data, phoneInput.value, true);
					if (!phoneDuplicateActive(data)) {
