...
(function () {
	...
	// Map, не WeakMap: ключ — строка e-mail
	...
	function contextFor(inp) {
		...
		// Регистрация при оформлении заказа (NEW_EMAIL) — не checkout
		if (formEarly && (
			formEarly.name === 'order_reg_form'
			|| formEarly.querySelector('input[name="do_register"][value="Y"]')
		) && (inp.name || '').toUpperCase() === 'NEW_EMAIL') {
			return 'signup';
		}
	...
	/** Полностью убрать предупреждение (политика / дубликат) */
	function clearNoticeBox(inp) {
		var box = ensureBox(inp, false);
		if (!box) return;
		hideBox(box);
	...
	function bind() {
		...
		document.addEventListener('submit', function (e) {
			...
			var inputs = form.querySelectorAll('input[name="USER_EMAIL"], input[name="REGISTER[EMAIL]"], input[name="EMAIL"], input[name="NEW_EMAIL"]');
	...
	window.primeAlertsIsEmailAllowed = function (email) {
